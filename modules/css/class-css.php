 ...
 */
if( !class_exists( 'Business_Power_Css ' ) ){
	class Business_Power_Css extends Business_Power_Helper{
		...
		*/
		...
		protected static function generate_css( $dynamic_css ){
			...
			foreach( $dynamic_css as $css ){
				$selector = str_replace( '%s', '.' . BUSINESS_POWER_PREFIX, $css[ 'selector' ] );
				echo $selector; ?>{
					<?php
						foreach( $css[ 'props' ] as $prop => $setting ){
							...
							if( is_array( $setting ) ){
								...
								$value = business_power_get( $setting );
								if( 'font-family' == $prop ){
		...
		*/
		public static function print_styles(){
			?>
			<style type="text/css" media="all" id="business-power-styles">
