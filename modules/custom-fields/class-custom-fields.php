...
if( !class_exists( 'Business_Power_Custom_Fields' ) ){
	class Business_Power_Custom_Fields extends Business_Power_Helper{
		...
		*/
		...
		public static function render_field( $field ){
			...
			switch( $field[ 'type' ] ){
				...
				case 'radio':
					?>
					<div class="components-base-control__field business-power-meta-radio">
						<?php  
							self::label( $field );
				...
				case 'select':
					...
					?>
					<div class="components-base-control__field business-power-meta-select">
						<?php  
							self::label( $field );
							self::description( $field );
						?>
				...
				case 'checkbox':
					?>
					<div class="components-base-control__field business-power-meta-checkbox">
						<input 
							type="checkbox" 
				...
				default:
					?>
					<div class="components-base-control__field business-power-meta-text">
						<?php  
							self::label( $field );
							self::description( $field );
						?>
		...
		public function render_meta_box( $post, $box ){
			...
			if( !is_array( $box[ 'args' ] ) )
				return;
			wp_nonce_field( 'business_power_meta_nonce', 'business_power_name_meta_nonce' );
			?>
			<div class="business-power-meta-box-wrapper">
				<?php 
					$count = 0; 
					foreach( $box[ 'args' ] as $id => $field ): 
						$id = 'business-power-' . self::uglify( $id );
				?>
					<div id="<?php echo esc_attr( $id ); ?>-wrapper" class="components-base-control__field business-power-meta-box-item">
						<?php 
							$field[ 'id' ]    = $id;
							$field[ 'post' ]  = $post;
		...
		public function save( $post_id ){
			...
			if ( $is_autosave || $is_revision || empty( $p ) || ! isset(  $p[ 'business_power_name_meta_nonce' ] ) || ! wp_verify_nonce( $p[ 'business_power_name_meta_nonce' ], 'business_power_meta_nonce' )) {
				return;
			}
