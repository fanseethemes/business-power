...
if( !class_exists( 'Business_Power_Slider_Integration' ) ){
	class Business_Power_Slider_Integration{
		...
		public function __construct(){
			Business_Power_Customizer::add_custom_control( array(
			    'type'     => $this->type,
			    'class'    => 'Business_Power_Customizer_Slider_Control',
			    'sanitize' => 'absint',
			    'register_control_type' => true,
			));

			add_action( 'business_power_customize_register_start', array( $this, 'start' ), 10, 2 );
			add_action( 'business_power_customize_register_end', array( $this, 'end' ), 10, 2 );

			# Feed default values to the framework
			add_filter( 'business_power_customizer_after_set', array( $this, 'setup_default_value' ), 10 );
		}
				...
				foreach( $instance::$fields[ $this->type ] as $field ){
					foreach( array( 'mobile', 'tablet', 'desktop' ) as $device ){
						if( isset( $field[ 'default' ] ) ){
							$id = "business-power-{$field['id']}-{$device}";;
	...
	new Business_Power_Slider_Integration();
}