 ...
 */
if( !class_exists( 'Business_Power_Helper' ) ){
	class Business_Power_Helper{
		...
		*/
		...
		public static function get_blog_title(){
			$ib_blog_title = business_power_get( 'ib-blog-title' );
		...
		public static function primary_menu_fb( $arg ) {
		    ...
		    <ul>
		        <li>
		            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'business-power' ) ?></a>
		        </li>
		        <li><a href="#"><?php esc_html_e( 'About Us', 'business-power' ) ?></a></li>
		        <li><a href="#"><?php esc_html_e( 'Our Team', 'business-power' ) ?></a></li>
		        <li><a href="#"><?php esc_html_e( 'Quick Links', 'business-power' ) ?></a>
		        	<ul>
		        		<li><a href="#"><?php esc_html_e( 'Inner Engineering Programme', 'business-power' ) ?></a></li>
		        	</ul>
		        </li>
		        <li><a href="#"><?php esc_html_e( 'Contact Us', 'business-power' ) ?></a></li>
		    </ul>
		    <?php
		    if( !$arg[ 'echo' ] ){
		...
		public static function social_menu_fb( $arg ) {
		    ...
		    ?>
		    <ul class="business-power-demo-social-menu">
		...
		public static function get_comment_number(){
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="business-power-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'business-power' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'business-power' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'business-power' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'business-power' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'business-power' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'business-power' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		*/
		...
		public static function post_format_icon(){
			...
			<?php if( $icon ): ?>
				<a class="business-power-post-type-icon" href="<?php the_permalink() ?>">
		...
		public static function add_action( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ){
			add_action( "business_power_{$tag}", $function_to_add, $priority, $accepted_args );
		}		
		...
		public static function add_filter( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ){
			add_filter( "business_power_{$tag}", $function_to_add, $priority, $accepted_args );
		}
		...
		public static function get_sidebar_class( $classes ){
			...
			if( $page_template || is_search() || self::is_woo_single_page() || self::is_woo_product_category() ){
				$classes[] = 'business-power-no-sidebar';
			}else{
				$customizer_position = business_power_get( 'sidebar-position' ); 
		...
		public static function is_sidebar_active(){
			...
			return count( $cls ) > 0 && $cls[ 0 ] !== 'business-power-no-sidebar' ? true : false;
		}
		...
		public static function get_meta( $meta_key = false, $post_id = null,  $single = true  ){
			if( $meta_key ){
				$meta_key = "business-power-{$meta_key}";
		...
		* @package Business Power WordPress Theme
		...
		public static function schema( $type ) {
			...
			return apply_filters( "business_power_schema_{$type}", $schema );
		}
	}
}